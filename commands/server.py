    ...
    async def serverCommand(interaction: Interaction):
        with open("specialConfig.json", "r") as specialConfigFile:
            ...
            if interaction.user.id != 629711559899217950:
                embed = Embed(title=" ",description=f"**:x: {format(translate("error.no_permission", lang))}**",colour=15548997)
                await interaction.response.send_message(" ",embed=embed, ephemeral=True)
                log(f"(FAILED) {interaction.user} FAILED to change the server (not allowed)")
                return
        
            if interaction.guild.id == server:
                embed = Embed(title=" ",description=f"**:x: {format(translate("cmd.error.server.same", lang))}**",colour=15548997)
                await interaction.response.send_message(" ",embed=embed, ephemeral=True)
                ...
                try:
                    ...
                    embed = Embed(title=" ",description=f"**:white_check_mark: {format(translate("cmd.server.success", lang))}** ``{interaction.guild}``!",colour=2067276)
                    await interaction.response.send_message(" ",embed=embed)
                    log(f"(SUCCESS) Server has been CHANGED")
                except:
                    embed = Embed(title=" ",description=f"**:x: {format(translate("error.generic", lang))}**",colour=15548997)
                    await interaction.response.send_message(" ",embed=embed, ephemeral=True)
